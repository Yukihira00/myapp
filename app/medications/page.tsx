...
export default function MedicationsPage(): React.JSX.Element {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#FAF7F2', display: 'flex', justifyContent: 'center', fontFamily: 'Nunito, sans-serif' }}>
      <div style={{ width: '100%', maxWidth: 420, height: '100vh', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        <Header />
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          <button 
            ...
            style={{ width: '100%', padding: '14px', borderRadius: 12, backgroundColor: '#7CB88A', border: 'none', color: '#FFFFFF', fontSize: 14, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 8, boxShadow: '0 4px 12px rgba(124, 184, 138, 0.3)' }}
          >
            <PlusIcon /> 新しいお薬を登録
          </button>

          {isLoading ? (
            <p style={{ textAlign: 'center', color: '#8A8278', fontSize: 13, marginTop: 20 }}>読み込み中...</p>
          ) : medications.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px 0', color: '#C4BDB3' }}>
              <CapsuleIcon size={40} color="#E5E7EB" />
              <p style={{ fontSize: 13, marginTop: 12 }}>登録されているお薬はありません</p>
            </div>
          ) : (
            medications.map((med) => (
              <div 
                ...
                style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: '16px', border: '1px solid #E5E7EB', boxShadow: '0 2px 8px rgba(0,0,0,0.02)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer' }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 36, height: 36, backgroundColor: '#FAF7F2', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  ...
                  <div>
                    <h3 style={{ fontSize: 15, fontWeight: 700, color: '#2A2420', margin: 0 }}>{med.name}</h3>
                    <p style={{ fontSize: 12, color: '#8A8278', margin: '4px 0 0' }}>デフォルト: {med.default_amount} 錠</p>
                  </div>
                </div>
          ...
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(42, 36, 32, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20, backdropFilter: 'blur(2px)' }}>
            <div style={{ backgroundColor: '#FFFFFF', borderRadius: 24, width: '100%', maxWidth: 340, padding: 24, boxShadow: '0 10px 25px rgba(0,0,0,0.1)' }}>
              ...
              <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#8A8278', marginBottom: 6 }}>アイコン</label>
                  <div style={{ display: 'flex', gap: 12 }}>
                    ...
                    >
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: iconType === 'tablet' ? '#2A2420' : '#8A8278' }}>錠剤</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span style={{ fontSize: 12, fontWeight: 700, color: iconType === 'capsule' ? '#2A2420' : '#8A8278' }}>カプセル</span>
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: '#8A8278', marginBottom: 6 }}>デフォルトの量</label>
                  <div style={{ position: 'relative' }}>
                    <select
                      value={amount}
                      onChange={(e) => setAmount(e.target.value)}
                      style={{ width: '100%', borderRadius: 12, border: '1px solid #E5E7EB', backgroundColor: '#FAF7F2', padding: '12px 14px', fontSize: 14, color: '#2A2420', appearance: 'none', cursor: 'pointer' }}
                      required
                    >
                    ...
                    <div style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }}>
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#8A8278" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>
                    </div>
                  </div>
                  <p style={{ fontSize: 11, color: '#C4BDB3', marginTop: 6, lineHeight: 1.4 }}>
