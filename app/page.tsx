  ...
  const handleSignOut = async () => {
    if (confirm('ログアウトしますか？')) {
      try {
        ...
        // 2. 画面の状態（ステート）をすべて強制空にする
        setSession(null);
        setTimelineLogs([]);
        setDailyAverages({});
        
        // 3. 【最重要】ブラウザの記憶（トークン残骸）を物理的に全消去
        localStorage.clear();
        sessionStorage.clear();
        
        // 4. 【最重要】URLの「#」以降のゴミを完全に排除した純粋なトップURLへ強制送還
        window.location.assign(window.location.origin);
      } catch (error: unknown) {
        let errMsg = '不明なログアウトエラー';
        if (error && typeof error === 'object' && 'message' in error) {
          errMsg = (error as SupabaseCustomError).message;
        }
        console.error('ログアウト失敗:', errMsg);
      }
    }
  };
