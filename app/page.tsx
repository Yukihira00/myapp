...
const SCORE_COLORS = [
  ...
  { bg: "#6B5840", text: "#FFFFFF" },
  { bg: "#9B8464", text: "#FFFFFF" },
  { bg: "#B8A882", text: "#2A2A2A" },
  { bg: "#C8D878", text: "#2A2A2A" },
  { bg: "#D8EC96", text: "#2A2A2A" },
  { bg: "#E8F5B0", text: "#2A2A2A" },
  { bg: "#F5FAD0", text: "#2A2A2A" },
];
...
export default function Home(): React.JSX.Element {
  ...
  const { modal, showAlert, showConfirm, showPrompt, handlePromptChange } =
    useAppModal();
  ...
  useEffect(() => {
    ...
    async function fetchDailyAverages() {
      ...
      if (!error && data) {
        ...
        (data as DailyMoodAverageRow[]).forEach((row) => {
  ...
  useEffect(() => {
    ...
    async function fetchTimelineData() {
      ...
      const formattedLogs: TimelineLog[] = (moodData as MoodLogResponse[]).map(
        (mood) => {
          const timeStr = new Date(mood.created_at).toLocaleTimeString(
            "ja-JP",
          ...
          const matchedMeds = typedMedData
            ? typedMedData
                .filter(
                  (m) =>
                    new Date(m.logged_at).toLocaleTimeString("ja-JP", {
                      hour: "2-digit",
                      minute: "2-digit",
                    }) === timeStr,
                )
                .map(
                  (m) => `${m.medications?.name || "不明な薬"} (${m.amount}錠)`,
                )
            : [];
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await showAlert(`認証エラー: ${(error as SupabaseCustomError).message}`);
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (targetDate > now) {
        await showAlert("エラー：未来の日時指定はできません。");
      ...
      await showAlert("同期完了しました");
      setSelectedScore(null);
      setMemo("");
      setSelectedMedIds([]);
      setIsOpen(false);
    } catch (error: unknown) {
      await showAlert(`同期失敗: ${(error as SupabaseCustomError).message}`);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleToggleMedId = (id: string) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p style={{ fontSize: 12, color: "#8A8278", margin: "0 0 24px" }}>
            感情と服薬の相関ライフログ
          </p>
          <form
            onSubmit={handleAuth}
      ...
      >
        ...
        {isOpen && (
          <AddEntryModal
            ...
            onToggleMedId={handleToggleMedId}
            memo={memo}
